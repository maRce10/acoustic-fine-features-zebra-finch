...
We try 9 methods of measuring acoustic structure:
...
(dtw_wv_mod <- readRDS("./data/processed/matrix_correlation_dtw_distnace.RDS"))
```
...
```{r}
(rav_mod <- readRDS("./data/processed/matrix_correlation_raven_measurements_distance.RDS"))
```
...
mod <- MRM2(formula = wrbl_sp ~ frequency + components + sign, nperm = 10000, mat = rect_var)

saveRDS(
  wrbl_mod,
  "./data/processed/matrix_correlation_warbler_measurements_distance.RDS"
)
```

```{r}

(wrbl_mod <- readRDS("./data/processed/matrix_correlation_warbler_measurements_distance.RDS"))
```
...
saveRDS(xc, "./data/processed/fourier_cross_correlation_schroeder.RDS")
```
...
saveRDS(
  xc_mod,
  "./data/processed/matrix_correlation_fourier_cross_correlation.RDS"
)
```

```{r}

(xc_mod <- readRDS("./data/processed/matrix_correlation_fourier_cross_correlation.RDS"))
```

## Raven's fourier cross-correlation
```{r, eval = FALSE}

xc_rav <- Rraven::imp_corr_mat(file = "schroeders_raven_fourier_cross_correlation.txt", path = "./data/processed")[[1]]
...
saveRDS(
  xc_rav_mod,
  "./data/processed/matrix_correlation_raven_fourier_cross_correlation.RDS"
)
...
(xc_rav_mod <- readRDS("./data/processed/matrix_correlation_raven_fourier_cross_correlation.RDS"))
```

## Raven's waveform correlation
```{r, eval = FALSE}

wav_cr_rav <- Rraven::imp_corr_mat(file = "schroeders_raven_waveform_correlation.txt", path = "./data/processed")[[1]]
...
saveRDS(
  wav_cr_rav_mod,
  "./data/processed/matrix_correlation_raven_waveform_correlation.RDS"
)
...
(wav_cr_rav_mod <- readRDS("./data/processed/matrix_correlation_raven_waveform_correlation.RDS"))
```
...
saveRDS(xc, "./data/processed/mel_frequency_cross_correlation_schroeder.RDS")
```
...
saveRDS(
  mxc_mod,
  "./data/processed/matrix_correlation_mel_cross_correlation.RDS"
)
```

```{r}

(mxc_mod <- readRDS("./data/processed/matrix_correlation_mel_cross_correlation.RDS"))
```
...
saveRDS(dtw_dists, "./data/processed/dtw_distance_dominant_frequency_schroeder.RDS")
```
...
saveRDS(
  dtw_dom_mod,
  "./data/processed/matrix_correlation_dtw_distances_dominant_frequency_contours.RDS"
)
```

```{r}

(dtw_dom_mod <- readRDS("./data/processed/matrix_correlation_dtw_distances_dominant_frequency_contours.RDS"))
```
...
saveRDS(rav_dtw_dists, "./data/processed/dtw_distance_raven_dominant_frequency_schroeder.RDS")
```
...
saveRDS(
  dtw_rav_peak_mod,
  "./data/processed/matrix_correlation_dtw_distances_raven_dominant_frequency_contours.RDS"
)
```

```{r}

(dtw_rav_peak_mod <- readRDS("./data/processed/matrix_correlation_dtw_distances_raven_dominant_frequency_contours.RDS"))
```


## Combined results
```{r, fig.cap = "Effect sizes per model and predictor", out.width="100%"}

mods <- list(dtw_wv_mod = dtw_wv_mod, wrbl_mod = wrbl_mod, rav_mod = rav_mod, xc_mod = xc_mod, mxc_mod = mxc_mod, dtw_dom_mod = dtw_dom_mod, dtw_rav_peak_mod = dtw_rav_peak_mod, wav_cr_rav_mod = wav_cr_rav_mod, xc_rav_mod = xc_rav_mod)

names(mods) <- c("DTW waveform period", "warbleR's spectro features", "Raven's spectro features", "warbleR's Fourier cross-correlation", "Mel cross-correlation", "DTW warbleR's dominant freq", "DTW Raven's peak freq", "waveform correlation", "Raven's Fourier cross-correlation")
...
- Dynamic time warping dissimilarity but no other metrics capture sign differences in Schroeders
