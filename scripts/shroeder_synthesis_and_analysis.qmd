...
- [warbleR cross-correlation of fourier spectrograms](#warblers-fourier-cross-correlation)
- [warbleR cross-correlation of mel-frequency spectrograms](#warblers-mel-frequency-cross-correlation)
...
mod <- MRM2(formula = dtw_dist ~ frequency + components + sign, nperm = 10000, mat = rect_var)
...
mod <- MRM2(formula = rav_dist ~ frequency + components + sign, nperm = 10000, mat = rect_var)
...
rav_mod <- readRDS("./data/processed/matrix_correlation_warbler_measurements_distance.RDS")
...
mod <- MRM2(formula = fourier_xc ~ frequency + components + sign, nperm = 10000, mat = rect_var)
...
mod <- MRM2(formula = mel_xc ~ frequency + components + sign, nperm = 10000, mat = rect_var)
...
mod <- MRM2(formula = dtw_dists ~ frequency + components + sign, nperm = 10000, mat = rect_var)
...
rect_var <- cbind(as.dist(rav_dtw_dists), freq_bi_tri, comp_bi_tri, sign_bi_tri)

colnames(rect_var) <- c("rav_dtw_dists", "frequency", "components", "sign")

source("~/Dropbox/Projects/geographic_call_variation_yellow-naped_amazon/scripts/MRM2.R")

mod <- MRM2(formula = rav_dtw_dists ~ frequency + components + sign, nperm = 10000, mat = rect_var)
...
(dtw_dom_mod <- readRDS("./data/processed/matrix_correlation_dtw_distances_raven_dominant_frequency_contours.RDS"))
